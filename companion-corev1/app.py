...
def chat():
    ...
    # Construir el contexto con los últimos mensajes del historial (ej: últimos 6 mensajes)
    historial_reciente = chat_history[-6:]
    contexto_conversacion = ""
    for h in historial_reciente:
        remitente = "Usuario" if h["sender"] == "user" else char["name"]
        contexto_conversacion += f"{remitente}: {h['text']}\n"

    # Prompt estructurado incluyendo el historial reciente para evitar que divague
    prompt_completo = (
        f"{char['system_prompt']}\n\n"
        f"[ESTADO ACTUAL DE AFECTO: {sesion_activa['afecto_actual']}]\n\n"
    ...
    # Guardar mensajes automáticamente en el historial
